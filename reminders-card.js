...
if (!window.customCards.some((card) => card.type === "reminders-card")) {
  window.customCards.push({
    type: "reminders-card",
    name: "Reminders Card",
    preview: false,
    description: "Add, edit, snooze, stop, and toggle reminders with an iPad-inspired layout.",
  });
}
class RemindersCard extends LitElement {
  ...
  constructor() {
    ...
    this._pendingPressReminder = null;
  ...
  getCardSize() {
    const reminders = this._getReminderEntities();
    return Math.max(reminders.length || 1, 2);
  }
  ...
  const settings = this._getIntegrationSettings();
        ...
        const reminders = this._buildReminderModels();
    return html`
      <ha-card>
        <div class="header">
          <div class="title-row">
          ...
          <button class="fab top" type="button" @click=${() => this._openDialog("create")} aria-label="Add reminder">
        ...
        ${settingsMissing
          ...
          : nothing}
        ${reminders.length
          ? html`<div class="reminder-grid">${repeat(reminders, (reminder) => reminder.entity_id, (reminder) =>
                this._renderReminderCard(reminder, pressMode)
              )}</div>`
          : html`<div class="empty-state">
              <div class="empty-icon">
                <ha-icon icon="mdi:bell-plus"></ha-icon>
              </div>
              <div class="empty-title">No reminders yet</div>
              <div class="empty-body">Tap the + button to schedule your first reminder.</div>
            </div>`}
      </ha-card>
      ${this._renderDialog()}
  ...
  _renderReminderCard(reminder, pressMode) {
    ...
    return html`
      <div
        class=${classes}
        @pointerdown=${(ev) => this._onPointerDown(ev, reminder)}
        @pointerup=${(ev) => this._onPointerUp(ev, reminder)}
        @pointercancel=${this._clearPendingPress}
        @pointerleave=${this._clearPendingPress}
        tabindex="0"
        @keydown=${(ev) => this._handleKeydown(ev, reminder)}
        role="button"
        aria-label=${`${reminder.displayTime} reminder card`}
      >
        <div class="card-top">
          <div class="card-head-left">
            <div class="time-row">
              <span class="time">${reminder.displayTime}</span>
              <span class="ampm">${reminder.ampm}</span>
              <span class=${classMap({ "status-chip": true, [`status-${reminder.status}`]: true })}>${reminder.statusLabel}</span>
            </div>
            <div class="date-row">
              <span class=${classMap({ date: true, placeholder: !reminder.dateLabel })}>${reminder.dateLabel || "\u00a0"}</span>
              ${reminder.status?.toLowerCase?.() === "scheduled"
                ? html`<button
                    class="skip-button"
                    type="button"
                    title="Skip next reminder occurrence"
                    aria-label="Skip next reminder occurrence"
                    @click=${(ev) => {
                      ev.stopPropagation();
                      this._callStop(reminder);
                    }}
          ...
          <div class="card-actions">
            <button
              ...
              @click=${(ev) => {
                ev.stopPropagation();
                this._openDialog("edit", reminder);
              }}
              @pointerdown=${(ev) => ev.stopPropagation()}
            >
        ...
        <div class="name-row">
          ${reminder.displayName
            ? html`<div class="name">${reminder.displayName}</div>`
            : html`<div class="name unnamed">&nbsp;</div>`}
        </div>
        <div class="meta">
          <div class="meta-row">
            <span class="label">Repeat:</span>
            <span class="value">${reminder.repeatLabel}</span>
          </div>
          <div class="meta-row">
            <span class="label">Player:</span>
            <span class="value">${reminder.mediaPlayerLabel}</span>
          </div>
          <div class="meta-row">
            <span class="label">Media:</span>
            <span class="value">${reminder.soundTitle || reminder.soundLabel}</span>
          </div>
        </div>
      </div>
    `;
      ...
      <div class="dialog-backdrop" @click=${this._handleDialogCancel}>
        <div class="dialog-panel" role="dialog" aria-modal="true" @click=${(ev) => ev.stopPropagation()}>
              ...
              <label class="field span-2">
                ...
                <div class="media-input-group">
                  <input
                    ...
                    placeholder="media-source://media_source/local/Reminders/chime.mp3"
                  ...
                  <button
                    ...
                    ?disabled=${!this._formData.media_player}
          ...
          <div class="dialog-actions">
            ${isEdit
              ? html`<button class="danger" type="button" ?disabled=${this._saving} @click=${this._deleteReminder}>
                  Delete
                </button>`
              : html`<span></span>`}
            <div class="dialog-actions-right">
              ...
              <button class="primary" type="button" ?disabled=${this._saving} @click=${this._submitForm}>
                ${this._saving ? "Saving..." : isEdit ? "Save changes" : "Create reminder"}
              </button>
            </div>
          </div>
        </div>
  ...
  _mapMediaSourceSearchResponse(response) {
    ...
    for (const candidate of candidates) {
      ...
      const item = {
        can_play: candidate.can_play !== undefined ? !!candidate.can_play : true,
        can_expand: !!candidate.can_expand && !candidate.can_play,
        media_content_id: id,
  ...
  async _callMediaBrowser(descriptor, targetOverride = null) {
    ...
    if (normalizedId && !normalizedType) {
      if (normalizedId.startsWith("media-source://")) {
        return await this.hass.callWS({
          type: "media_source/browse_media",
          media_content_id: normalizedId,
        });
      }
    }
    if (normalizedId) {
    ...
    try {
      return await this.hass.callWS(request);
    } catch (err) {
      const fallbackId = normalizedId || (descriptor && typeof descriptor === "object" ? descriptor.id : undefined);
      if (fallbackId && typeof fallbackId === "string" && fallbackId.startsWith("media-source://")) {
        console.warn("media_player/browse_media failed, trying media_source", err);
        return await this.hass.callWS({
          type: "media_source/browse_media",
          media_content_id: fallbackId,
        });
      }
      throw err;
    }
  ...
  _selectMediaItem(item) {
    ...
    const displayTitle = descriptor.media_content_title || descriptor.title || formatMediaName(item.media_content_id);
  ...
  _descriptorLooksLikePlex(descriptor, item = null) {
    ...
    const looksLikePlex = Boolean(
      (provider && provider.includes("plex")) || lowerId.startsWith("media-source://plex/") || lowerId.startsWith("plex://")
    );
    this._debugPlex("Descriptor Plex detection result", {
      descriptor: this._summarizePlexDescriptor(descriptor, item),
      looksLike: looksLikePlex,
    });
    return looksLikePlex;
  }
  ...
  _descriptorLooksLikeJellyfin(descriptor, item = null) {
    ...
    const mediaId = descriptor?.media_content_id || descriptor?.id || item?.media_content_id || item?.id;
    const lowerId = typeof mediaId === "string" ? mediaId.toLowerCase() : "";
    const looksLikeJellyfin = Boolean(provider.includes("jellyfin") || lowerId.startsWith("media-source://jellyfin/"));
    this._debugJellyfin("Descriptor Jellyfin detection result", {
      descriptor: this._summarizeJellyfinDescriptor(descriptor, item),
      looksLike: looksLikeJellyfin,
    });
    return looksLikeJellyfin;
  }
  ...
  _openDialog(mode, reminder, event) {
    if (event) {
    ...
    if (mode === "edit" && reminder) {
      this._formData = this._prefillFromReminder(reminder);
    } else {
      this._formData = this._blankForm();
    }
  ...
  _blankForm() {
    const settings = this._getIntegrationSettings();
    const defaultSound = settings.default_reminder_sound || "";
    return {
      reminder_id: null,
      name: "",
      message: "",
  ...
  _prefillFromReminder(reminder) {
    const stateObj = this.hass?.states?.[reminder.entity_id];
    const attrs = stateObj?.attributes || {};
    const scheduledSource = attrs.scheduled_time || reminder.scheduledTime;
    ...
    const mediaPlayer = this._normalizeMediaPlayerValue(
      attrs.media_player ||
        ...
        reminder.media_player ||
        reminder.media_player_id ||
        reminder.media_player_entity_id ||
        reminder.mediaPlayer
    );
    const repeatSource = attrs.repeat !== undefined ? attrs.repeat : reminder.repeat;
    const repeat = this._normalizeRepeatValue(repeatSource);
    const repeatDaysSource = attrs.repeat_days !== undefined ? attrs.repeat_days : reminder.repeat_days;
    ...
    const soundDescriptor =
      typeof attrs.sound_media === "object" && attrs.sound_media
        ...
        : undefined;
    const soundMediaId =
      (soundDescriptor && soundDescriptor.media_content_id) || attrs.sound_media || reminder.sound_media_id || "";
    const soundMediaType =
      (soundDescriptor && soundDescriptor.media_content_type) || reminder.sound_media_type || "";
    ...
    const rawNameSource = typeof (attrs.name ?? reminder.name) === "string" ? (attrs.name ?? reminder.name) : "";
    ...
    return {
      ...
      name: nameForInput,
      message: attrs.message ?? reminder.message ?? "",
  ...
  _updateFormField(field, value) {
    ...
    if (
      ...
      !this._suppressJellyfinMetadataUpdate &&
  ...
  async _submitForm() {
    ...
    try {
      ...
      if (this._dialogMode === "edit" && this._formData.reminder_id) {
        await this._callIntegrationService("edit_reminder", payload);
        this._notify("Reminder updated");
      } else {
        await this._callIntegrationService("set_reminder", payload);
        this._notify("Reminder created");
      }
      this._closeDialog();
    } catch (err) {
  this._dialogError = err?.message || "Failed to save reminder";
      this._saving = false;
    }
  }
  async _deleteReminder() {
    if (!this._formData.reminder_id || this._saving) {
      return;
    }
    ...
    this._saving = true;
    try {
      await this._callIntegrationService("delete_reminder", {
        reminder_id: this._formData.reminder_id,
      });
      this._notify("Reminder deleted");
      this._closeDialog();
    } catch (err) {
      this._dialogError = err?.message || "Failed to delete reminder";
      this._saving = false;
    }
  }
  _validateForm() {
    ...
    if (!this._formData.name || !this._formData.name.trim()) {
      errors.push("Enter a reminder name");
    }
  ...
  _buildServicePayload() {
    ...
    const isEdit = this._dialogMode === "edit" && this._formData.reminder_id;
  ...
  _mediaPlayerOptions() {
    ...
    return Object.values(this.hass.states)
      ...
      .filter((state) => !this._isSpotifyPlatformPlayer(state.entity_id))
      .map((state) => {
  ...
  _activationOptions() {
    ...
    const dynamic = this._buildReminderModels()
      .map((reminder) => reminder.activation_entity)
      .filter(Boolean);
  ...
  _isSpotifyPlatformPlayer(entityId) {
    if (!entityId) {
      return false;
    }
  ...
  _isDefaultReminderName(value) {
    return typeof value === "string" && /^reminder_\d+$/i.test(value.trim());
  }
  ...
  _formatReminderDisplayName(value) {
  ...
  _getReminderEntities() {
    ...
    return Object.values(this.hass.states).filter(
      (state) => state?.entity_id && this._isReminderEntityId(state.entity_id)
    );
  }
  _buildReminderModels() {
    const reminders = this._getReminderEntities().map((state) => {
      ...
      const autoName = rawName ? this._isDefaultReminderName(rawName) : false;
      const displayName = autoName ? "" : this._formatReminderDisplayName(baseName);
  ...
  _handleKeydown(ev, reminder) {
    if (ev.key === "Enter" || ev.key === " ") {
      ev.preventDefault();
      reminder.isActive ? this._handleActiveShort(reminder) : this._toggleSwitch(reminder);
    } else if (ev.key === "e") {
      ev.preventDefault();
      this._openDialog("edit", reminder);
    }
  }
  _onPointerDown(ev, reminder) {
    if (ev.button !== 0) {
      return;
    }
    ev.preventDefault();
    this._pendingPressReminder = reminder;
    this._pressTriggered = false;
    this._pressHandle = window.setTimeout(() => {
      this._pressTriggered = true;
      this._pressHandle = null;
      this._handleLongPress(reminder);
    }, this._config.long_press_ms ?? LONG_PRESS_DEFAULT_MS);
  }
  _onPointerUp(ev, reminder) {
    ev.preventDefault();
    if (this._pressHandle) {
      clearTimeout(this._pressHandle);
      this._pressHandle = null;
    }
    if (!this._pressTriggered && this._pendingPressReminder?.entity_id === reminder.entity_id) {
      this._handleShortPress(reminder);
    }
    this._pendingPressReminder = null;
  }
  _clearPendingPress = () => {
  ...
  _handleShortPress(reminder) {
    if (reminder.isActive) {
      this._handleActiveShort(reminder);
    } else {
      this._toggleSwitch(reminder);
    }
  }
  _handleLongPress(reminder) {
    if (reminder.isActive) {
      this._handleActiveLong(reminder);
    } else {
      this._openDialog("edit", reminder);
    }
  }
  _handleActiveShort(reminder) {
    ...
    if (mode.short === "Stop") {
      ...
      this._callSnooze(reminder);
    }
  }
  _handleActiveLong(reminder) {
    const mode = this._getPressMode();
    if (!mode) {
      return;
    }
  ...
  async _toggleSwitch(reminder) {
    const entityId = this._reminderSwitchEntityId(reminder.object_id);
    const service = reminder.isEnabled ? "turn_off" : "turn_on";
    try {
      await this.hass.callService("switch", service, { entity_id: entityId });
      this._notify(reminder.isEnabled ? "Reminder disabled" : "Reminder enabled");
    } catch (err) {
      this._notify("Unable to toggle reminder", true);
    }
  }
  async _callStop(reminder) {
    try {
      await this._callIntegrationService("stop_reminder", {
        reminder_id: reminder.entity_id,
      });
      this._notify("Reminder stopped");
    } catch (err) {
      this._notify("Failed to stop reminder", true);
    }
  }
  async _callSnooze(reminder) {
    try {
      const data = { reminder_id: reminder.entity_id };
      const snoozeMinutes = this._getSnoozeMinutes();
      if (snoozeMinutes) {
        data.minutes = snoozeMinutes;
      }
      await this._callIntegrationService("snooze_reminder", data);
      this._notify("Reminder snoozed");
    } catch (err) {
      this._notify("Failed to snooze reminder", true);
    }
  }
  ...
  _isReminderEntityId(entityId) {
    if (!entityId) {
      return false;
    }
    return entityId.startsWith(`${INTEGRATION_DOMAIN}_reminder.`);
  }
  _reminderSwitchEntityId(objectId) {
  ...
  _readJellyfinDebugPreference() {
    try {
      ...
      const value = window.localStorage.getItem("remindersCardJellyfinDebug");
      if (value === null) {
        return false;
      }
      ...
      console.warn("Unable to read Reminders Jellyfin debug preference", err);
      return false;
    }
  }
  _debugJellyfin(message, data = undefined) {
    ...
    const label = "[reminders-card][jellyfin]";
    if (data !== undefined) {
  ...
  _readPlexDebugPreference() {
    try {
      ...
      const value = window.localStorage.getItem("remindersCardPlexDebug");
      if (value === null) {
        return false;
      }
      const lowered = value.toLowerCase();
      ...
      console.warn("Unable to read Reminders Plex debug preference", err);
      return false;
    }
  }
  _debugPlex(message, data = undefined) {
    ...
    const label = "[reminders-card][plex]";
    if (data !== undefined) {
      console.debug(label, message, data);
    } else {
      console.debug(label, message);
  ...
  _readDlnaDebugPreference() {
    try {
      ...
      const value = window.localStorage.getItem("remindersCardDlnaDebug");
      if (value === null) {
        return false;
      }
      const lowered = value.toLowerCase();
      ...
      console.warn("Unable to read Reminders DLNA debug preference", err);
      return false;
    }
  }
  _debugDlna(message, data = undefined) {
    ...
    const label = "[reminders-card][dlna]";
    if (data !== undefined) {
      console.debug(label, message, data);
    } else {
      console.debug(label, message);
  ...
  column-gap: 12px;
      ...
      .card-head-left {
        --_time-width: var(--reminder-card-time-width, var(--alarm-card-time-width, 74px));
        --_ampm-width: var(--reminder-card-ampm-width, var(--alarm-card-ampm-width, 32px));
        --_status-min: var(--reminder-card-status-min-width, var(--alarm-card-status-min-width, 84px));
      ...
      .reminder-card.disabled .skip-button {
        background: rgba(0, 0, 0, 0.1);
      }
      .reminder-card.disabled .skip-button:hover {
      ...
      .time-row {
        display: grid;
        grid-template-columns: var(--reminder-card-time-width, var(--alarm-card-time-width, 74px))
          var(--reminder-card-ampm-width, var(--alarm-card-ampm-width, 32px))
          minmax(var(--reminder-card-status-min-width, var(--alarm-card-status-min-width, 84px)), auto);
        column-gap: 6px;
        align-items: center;
      }
      .date-row {
      ...
      .status-chip {
        ...
        justify-content: center;
        align-items: center;
  min-width: var(--reminder-card-status-min-width, var(--alarm-card-status-min-width, 96px));
        justify-self: start;
      }
...
class RemindersCardEditor extends LitElement {
  static get properties() {
    return {
      hass: {},
      _config: { state: true },
...
if (!customElements.get("reminders-card")) {
  customElements.define("reminders-card", RemindersCard);
}
if (!customElements.get("reminders-card-editor")) {
  customElements.define("reminders-card-editor", RemindersCardEditor);
}
RemindersCard.getStubConfig = () => ({
  title: "Reminders",
});
RemindersCard.getConfigElement = () => {
  return document.createElement("reminders-card-editor");
};
